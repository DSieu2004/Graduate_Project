...
if __name__ == '__main__':
    # 1. Cho thằng Web Server API chạy ở luồng phụ (ẩn phía sau)
...
# # ==========================================================
# app = Flask(__name__)
# CORS(app)  # Cho phép Web GUI (index.html) kết nối từ bất kỳ origin nào

# # [SỬA HIGH]: Dùng threading.Lock() để bảo vệ biến latest_qr.
# # Flask thread (đọc) và ROS thread (ghi) chạy đồng thời →
# # nếu không có lock, 2 luồng tranh nhau có thể làm mất dữ liệu QR.
# latest_qr  = ""
...
#     with qr_lock:
#         res       = latest_qr
#         latest_qr = ""  # Xóa sau khi Web lấy để không trả trùng lần sau
#     return jsonify({"data": res})
...
#         self.last_scanned_data      = ""
#         self.last_scan_time         = 0
#         self.COOLDOWN_TIME          = 3.0   # Giây — tránh quét trùng cùng 1 mã liên tục
#         self.last_frame_process_time = 0
#         self.PROCESS_INTERVAL       = 0.5   # Giây — chỉ xử lý 2 frame/giây để giảm tải CPU Pi
...
#         # Throttle: bỏ qua frame nếu chưa đến lúc xử lý
#         current_time = time.time()
#         if current_time - self.last_frame_process_time < self.PROCESS_INTERVAL:
#             return
#         self.last_frame_process_time = current_time

#         # Chuyển ROS Image → OpenCV frame
#         frame    = self.br.imgmsg_to_cv2(data, 'bgr8')
#         qr_codes = decode(frame)
...
#             # Chỉ lưu mã mới hoặc mã cũ nhưng đã qua cooldown
...
#                 # [SỬA HIGH]: Dùng lock khi ghi vào biến dùng chung
#                 with qr_lock:
#                     latest_qr = qr_data
...
#     # [SỬA CRITICAL]: Chỉ gọi rclpy.init() MỘT LẦN duy nhất ở đây.
#     # Trước đây có thêm 1 lần trong hàm run_ros() (dead code) →
#     # nếu vô tình gọi run_ros() sẽ crash vì init 2 lần.
#     rclpy.init()
#     node = MyQRReader()
...
#         )
#     )
#     flask_thread.daemon = True  # Tự tắt khi chương trình chính kết thúc
#     flask_thread.start()
